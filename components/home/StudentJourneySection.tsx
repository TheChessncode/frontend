...
import { ChevronRight, Star, Target, Trophy, User, Brain, Database, GraduationCap, Rocket } from "lucide-react";
import { getAllStudents } from "@/constants/studentsData";
import Link from "next/link";

export default function StudentJourneySection() {
  const students = getAllStudents();
  const [selectedStudentSlug, setSelectedStudentSlug] = useState(students[0]?.slug || "elora");
  const selectedStudent = students.find(s => s.slug === selectedStudentSlug) || students[0];
  ...
  const getLevelsForStudent = (student: typeof selectedStudent) => {
      ...
      return [
    {
      level: 1,
          ...
          icon: <GraduationCap className="w-5 h-5 text-[var(--text-secondary)]" />
    },
    {
      level: 2,
          title: "Data Analysis", 
    ...
    
    {
      ...
      return [
        ...
        },
        {
          ...
          status: "upcoming" as const, 
          skills: ["Statistics", "Excel", "Data Fundamentals", "Analysis Process"],
  ...
  const [currentLevel, setCurrentLevel] = useState(currentLevelIndex >= 0 ? currentLevelIndex + 1 : 1);

  // Update current level when student changes
  useEffect(() => {
    const newLevels = getLevelsForStudent(selectedStudent);
    const newCurrentLevelIndex = newLevels.findIndex(l => l.status === "current");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl lg:text-5xl font-bold text-[var(--text-primary)] mb-4">
            Meet Our <span className="text-[var(--brand-primary)]">Scholars</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-[var(--text-primary)]">{selectedStudent.name}</h3>
              <p className="text-[var(--text-secondary)]">{selectedStudent.chessBackground}</p>
              ...
              <div className="mt-4 p-3 bg-[var(--bg-tertiary)] rounded-lg">
                <p className="text-sm italic text-[var(--text-secondary)]">&quot;{selectedStudent.quote}&quot;</p>
              </div>
